...
export default class ContextMenu {
  ...
  @param {Graph}  graph the graph that the context menu applies to */
	constructor(graph) {
		...
		menu.devModeBox.addEventListener("change", () => {
			log.debug("Set devMode to " + menu.devModeBox.checked);
			this.populate(menu.devModeBox.checked, menu.extModeBox.checked);
		});
		menu.extModeBox.addEventListener("change", () => {
			log.debug("Set extMode to " + menu.extModeBox.checked);
			this.populate(menu.devModeBox.checked, menu.extModeBox.checked);
		});
	}
