  ...
  */
	constructor(graph, dev) {
		...
		Object.values(commands)
			.flat()
			.forEach((cmd) => logWrap(cmd, (edge) => `edge with property ${edge.data(EDGE.PROPERTY)} between ${edge.data(EDGE.SOURCE)} ${edge.data(EDGE.TARGET)}`));

		if (dev) {
			this.menus = [ContextMenuEdges.devMenu(commands), ContextMenuEdges.devLimesMenu(commands)];
		} else {
			this.menus = [ContextMenuEdges.baseMenu(commands), ContextMenuEdges.baseLimesMenu(commands)];
		}
	}
		...
		return [
			{
				...
				id: "edit",
				select: (edge) => {
					const body = `Problem with the edge [${this.edgeLabel(edge)}](${edge.data(EDGE.SOURCE)}) ([OntoWiki URL](${ontoWikiUrl(
			...
			},
			{
				...
				id: "description",
				select: (node) => {
					if (node.data(EDGE.AXIOM)) {
						window.open(node.data(EDGE.AXIOM));
					} else {
