...
function moveMatch(distance)
{
  for(let i=0; i < matchComponents.length; i++)
  {
    const comp = matchComponents[i];
...
function combineMatch(enabled)
{
  ...
  matchComponents.length=0;
  matchComponents.push(...matchGraph.components());
  for(let i=0; i < matchComponents.length; i++)
  {
    const comp = matchComponents[i];
    if(comp.length===1) {continue;}
    ...
    },
    {
      ...
      "entries":
            [
              ...
              [()=>moveMatch(0), "move matches on top of each other","move-match-on-top"],
              [()=>moveMatch(100), "move matches nearby","move-match-nearby"],
